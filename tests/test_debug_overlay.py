...
def test_debug_overlay_remains_inside_the_left_third_of_the_screen(display):
    session = Session(Viewport(WINDOW))
    ...
    outside = surface.get_at((surface.get_width() // 3 + 2, PANEL[1] + 2))[:3]
    assert outside != BACKGROUND[:3]
