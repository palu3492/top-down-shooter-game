...
def game_loop():
    ...
    while game_is_running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN and ammo_count not in ("no ammo", "reload"):
                human_anim = "SHOOT"
                bullet = Shot((window[0]/2.0)-camera_x, (window[1]/2.0)-camera_y, center_x, center_y)
                bullets.add(bullet)
            ...
            if event.type==pygame.KEYDOWN:
                ...
                if event.key==pygame.K_g and all_grenade_data.grenade_amount > 0:
                    grenade = Grenade((window[0] / 2.0) - camera_x, (window[1] / 2.0) - camera_y, center_x, center_y)
                    grenades.add(grenade)
                    all_grenade_data.grenade_amount -= 1
                if event.key==pygame.K_f and all_grenade_data.stun_grenade_amount>0:
                    stun_grenade = StunGrenade((window[0] / 2.0) - camera_x, (window[1] / 2.0) - camera_y, center_x, center_y)
